...
fn interactor_with_tooltip() {
    let _: E = text("hello").modify(
        Modifier::new()
            .padding(8)
            .on_press(Msg::A)
            .tooltip_text("Click me", tooltip::Position::Top),
    );
}
...
fn interactor_with_scrollable() {
    let _: E = column![text("a"), text("b")]
        .modify(Modifier::new().height(100).on_press(Msg::A).scrollable());
}
...
fn interactor_with_tooltip_and_scrollable() {
    let _: E = column![text("a"), text("b")].modify(
        Modifier::new()
            .height(100)
            .on_press(Msg::A)
            .tooltip_text("info", tooltip::Position::Bottom)
            .scrollable(),
    );
}
...
fn on_scroll_callback() {
    let _: E = text("hello").modify(Modifier::new().on_scroll(|_delta| Msg::A));
}

#[test]
fn on_move_callback() {
    let _: E = text("hello").modify(Modifier::new().on_move(|_point| Msg::A));
}
...
fn on_scroll_with_hidden() {
    let _: E = text("hello").modify(Modifier::new().on_scroll(|_delta| Msg::A).hidden(true));
}
