...
fn width_fixed_then_fill_width_last_wins() {
    let _: E = text("hello").modify(Modifier::new().width(Length::Fixed(100.0)).fill_width());
}

#[test]
fn fill_width_then_width_fixed_last_wins() {
    let _: E = text("hello").modify(Modifier::new().fill_width().width(Length::Fixed(100.0)));
}
...
fn corner_radius_then_border_overwrites_all() {
    let _: E = text("hello").modify(Modifier::new().corner_radius(8).border(Border {
        color: Color::BLACK,
        width: 2.0,
        radius: 0.into(),
    }));
...
fn border_color_then_border_width_accumulate() {
    let _: E = text("hello").modify(Modifier::new().border_color(Color::BLACK).border_width(2.0));
}
